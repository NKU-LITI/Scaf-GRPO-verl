...
def estimate_pass_at_k(vals: list[Any], k: int) -> float:
    """Compute unbiased pass@k from n sampled 0/1-style scores for one prompt."""
    n = len(vals)
    if n == 0:
        return 0.0
    k = min(k, n)
    correct = sum(float(val) > 0 for val in vals)
    if correct == 0:
        return 0.0
    if n - correct < k:
        return 1.0
...
def process_validation_metrics(
    ...
    Returns:
        ...
        - "pass@N/mean": Strict pass@N for 0/1-style scores, averaged across prompts
