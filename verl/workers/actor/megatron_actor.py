...
# [DEL] Legacy actor no longer calls `get_policy_loss_fn` directly.
# [ADD] Standard and Scaf policy-loss selection is centralized in the shared helper.
...
# [ADD] Use the Scaf-aware objective in verl 0.7's legacy Megatron actor path.
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                # [ADD] Masks produced by trainer-side hint/expert replacement.
                sft_loss_mask = data.get("sft_loss_mask")
                hint_sft_loss_mask = data.get("hint_sft_loss_mask")
                off_policy_mask = data.get("off_policy_mask")
                ...
                # [ADD] Evaluate the same mixed objective as the legacy FSDP actor.
                policy_loss, pg_loss, pg_metrics = compute_scaf_ppo_policy_loss(
                    config=self.config,
                ...
                # [DEL] `policy_loss = pg_loss` discarded Scaf auxiliary SFT terms.
                # [ADD] Keep the combined policy loss computed above.
            ...
            if calculate_entropy:
                ...
                if not forward_only:
                    ...
                    # [DEL] Rebuilding from `pg_loss` discarded Scaf auxiliary losses.
                    # policy_loss = pg_loss - entropy_coeff * entropy_loss
                    # [ADD] Apply entropy regularization to the combined Scaf objective.
