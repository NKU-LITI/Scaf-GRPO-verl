...
OUTPUT_DIR="${OUTPUT_DIR:-${RUN_DIR}/rollout_log/validation}"
...
for ckpt_path in "${ckpt_paths[@]}"; do
    ...
    hf_model_path="${ckpt_path}/huggingface"
    ...
    if [[ ! -f "${hf_model_path}/config.json" ]]; then
        echo "ERROR: Hugging Face checkpoint not found under ${hf_model_path}" >&2
        echo "Make sure SFT was run with trainer.checkpoint.save_contents including hf_model." >&2
        exit 1
    fi

    if [[ -e "${OUTPUT_DIR}/${step_num}.jsonl" ]]; then
        echo "Skip ${step_name}: validation trajectory already exists at ${OUTPUT_DIR}/${step_num}.jsonl"
        continue
    fi
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name="${RUN_DIR}_rollout_validation" \
        "$@"
    } 2>&1 | tee -a "${ckpt_log}"
done
